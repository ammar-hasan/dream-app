...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `dream.read_project`         | `path`                                                                                                                                                                                                                                                                                                                      | project summary: id, name, size, background, complete named project-color list, derived normal-text contrast ratio/AA status for each color, mode, layer count, frame count (null when unanimated), hotspot counts (total + broken), operation counts (total + per kind), game-setup flag, timestamps | read-only                                |
| `dream.set_project_color`    | `path`, `name` (1–40 characters), `value`; optional `color` (existing id or exact name; omit to add)                                                                                                                                                                                                                        | resulting id, name, normalized value and index                                                                                                                                                                                                                                                        | maximum 24; updates do not recolor marks |
| `dream.remove_project_color` | `path`, `color` (id or exact name)                                                                                                                                                                                                                                                                                          | removed color facts, prior index and remaining count                                                                                                                                                                                                                                                  |                                          |
...
| `dream.list_layers`          | `path`                                                                                                                                                                                                                                                                                                                      | per frame (or active stack): layer id, name, visibility, opacity, blend mode, complete editable adjustment settings, mask enabled/stroke count or null, lock, operation count                                                                                                                         |                                          |
...
| `dream.update_layer`         | `path`, `layer` (id or name); at least one of `name` (non-empty), `visible`, `opacity` (0–1), `blendMode` (`normal`, `multiply`, `screen`, `overlay`, `darken`, `lighten`), partial `adjustments` (the documented layer ranges), `mask` (`add`, `enable`, `disable`, `delete`), `locked`, `index` (zero-based; 0 is bottom) | the resulting complete layer facts including mask summary, stack index and active frame id                                                                                                                                                                                                            | active frame only; index must exist      |
...
| `dream.add_stroke`           | `path`, `points` (2–10,000 finite x/y samples; optional pressure 0–1); optional `tool` (brush default, pencil, eraser), `color` (default `#1f2937`), `size` (>0 to 8192, default 8), `opacity` (0–1, default 1), `layer` (id/name; default top active layer)                                                                | new operation id and target layer id/name                                                                                                                                                                                                                                                             | pencil/eraser are always fully opaque    |
| `dream.add_mask_stroke`      | `path`, `points` (2–10,000 finite x/y samples; optional pressure 0–1); optional `mode` (`hide` default, `reveal`), `size` (>0 to 8192, default 8), `opacity` (0–1, default 1), `layer` (id/name; default top active layer)                                                                                                  | new mask-stroke id, target layer id/name and mask-stroke count                                                                                                                                                                                                                                        | creates and enables the mask when needed |
...
2. It can inspect and maintain the same portable named project colors as a
   person, including the same canvas-background normal-text contrast guidance,
   and can add, rename, configure, blend, non-destructively adjust or mask,
   reorder and safely remove layers, then append pressure-aware freehand,
   mask, shape or text marks to the active frame.
   Animated-project edits preserve the active stack mirror. It deliberately
   cannot import raster content, edit hotspots/components or undo.
