...
| `dream.list_layers`    | `path`                                                                                                                                                                                                                                                       | per frame (or active stack): layer id, name, visibility, opacity, blend mode, lock, operation count                                                                                                   |                                          |
...
| `dream.update_layer`   | `path`, `layer` (id or name); at least one of `name` (non-empty), `visible`, `opacity` (0–1), `blendMode` (`normal`, `multiply`, `screen`, `overlay`, `darken`, `lighten`), `locked`, `index` (zero-based; 0 is bottom)                                      | the resulting layer id, name, visibility, opacity, blend mode, lock, operation count, stack index and active frame id                                                                                 | active frame only; index must exist      |
...
2. It can add, rename, configure, blend, reorder and safely remove layers, then append
   pressure-aware freehand strokes, shapes or text to the active frame.
