...
1. The user tells a story in English, Arabic, Persian, Simplified Chinese,
   Brazilian Portuguese or Russian. Speech recognition fills the same story box where the device supports it;
   the control simply hides where it does not. In Little Dreamer mode,
   finishing dictation immediately shows the plan without requiring a second,
   reading-dependent action.
