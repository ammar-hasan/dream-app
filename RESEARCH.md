...
1. **Kids navigate by recognition, not recall** — they click pictures, not
   ...
   *Dream:* Big pictorial toolbar, tooltips spoken aloud in kid mode, minimal
...
2. **Children's interfaces differ fundamentally from adults'** — adults scan
   ...
   *Dream:* Every accidental click is recoverable (full undo history); nothing
...
3. **Speech interfaces fit pre-literate children** — young kids can talk long
   ...
   *Dream:* Voice commands and dictation are first-class in kid mode, behind
...
4. **Immediate sensory feedback sustains engagement** — sounds and instant
   ...
   *Dream:* Tool sounds, spoken names, and instant canvas response on every
   stroke.
5. **Avoid text-heavy instruction and error states** — reading level gates
   ...
   *Dream:* No dialogs a child must read to continue; spoken names replace
...
6. **Larger targets, higher contrast, fewer modes** — motor precision and
   ...
   *Dream:* Design tokens control sizing/contrast; a senior comfort toggle is
...
7. **WCAG 2.5.5 Target Size (AAA): 44×44px minimum** — pointer targets must be
   ...
   *Dream:* Toolbar buttons already meet or exceed this; keep it a hard rule
...
8. **Reduce memory load: one thing at a time, visible state** — avoid hidden
   ...
   *Dream:* The active tool is always visually indicated; modes (Design /
...
9. **Plain language, short sentences, front-loaded meaning** — write for a
   ...
   *Dream:* All UI strings live in i18n tables written in plain words; keep
   them short.
10. **Icons must be concrete and familiar** — abstract icons fail; literal,
    ...
    *Dream:* Tool icons are literal (a pencil, an eraser, a bucket), and kid
...
11. **Pair text with audio and imagery** — multimodal redundancy compensates
    ...
    *Dream:* Speech synthesis (`say.ts`) can voice any label; locale-aware
...
|---|---|---|---|---|
| 1 | AI "make real" code export — turn an app-mode drawing into runnable code | Maria, Ahmed, Sara | 5 | M |
| 2 | More game templates + conversational game generation ("make a game where…") | Zainab, George | 5 | M |
| 3 | Stamps, stickers, and coloring-page starters | Zainab, George | 4 | S |
...
| 6 | Localized voice-command vocabularies, Arabic-first | Zainab, George, Ahmed | 4 | S |
| 7 | Senior comfort toggle (larger UI, higher contrast, slower animations) | Victor | 4 | S |
