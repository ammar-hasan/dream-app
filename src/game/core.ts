/**
 * Play mode: the "Catch!" game core. Framework-free and pure — no DOM, no
 * React, no imports from store/ or ui/ — so the whole game is unit-testable
 * in Node with a seeded RNG.
 *
 * One template, done well: things fall from the top, the player slides the
 * hero left/right to catch the good ones (+1 point) and dodge the bad ones
 * (-1 life). `tick` is the heart: state in, state out, with the events that
 * happened during the step (the UI turns those into pops, shakes and bleeps).
 */

import { mulberry32 } from '../engine/spray';
import type { DreamDocument, GameSettings, GameSetup } from '../engine/types';
...
export type ResolvedGameSetup = Required<GameSetup>;
