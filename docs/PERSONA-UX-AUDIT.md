...
- **Persona readiness: 6 of 10 fully ready, 4 partially ready.** Dream already
  gives every persona a credible path, but the universal “anyone” promise is
  stricter than a feature checklist. The largest gaps are data-driven
  scientific illustration, broader professional art/design depth, broader
  language reach and a publicly installable agent surface.
...
136 acceptance criteria), the product roadmap/research backlog, the automated
...
| Zǐxuān, 28    | Scientific symbols, reversible-reaction connectors, precise layout, scalable SVG, animation, presentations, app links and agent-readable projects               | He can author a real labeled reaction diagram, animate it and turn explanatory frames into an app, but grids, plots, data import and chemistry structures still block a broad research workflow | Partial   |
...
5. **Scientific creation is useful but not yet a research system.** Reaction
   arrows, common notation and SVG cover hand-authored diagrams; plots, grids,
   structured chemistry and data-driven figures remain manual or unavailable.
...
2. Deepen the scientific path with grids and small data-driven plots before
   attempting a general-purpose vector editor or chemistry suite.
