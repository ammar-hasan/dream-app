...
| Aleksandr, 25 | Move at professional thought speed: direct manipulation, precise inspectors, reusable systems, responsive layout, powerful shortcuts, extensibility and polished handoff without tool friction.   | Predictive pointer/drag states, keyboard workflows, snapping/alignment, groups/components, layers, localized UI, batch delivery. | No vector paths, auto layout/constraints, variables/tokens, linked components/variants, grids/rulers, multi-page organization, extensibility, inspector depth or drag ghosts.                                 |   4.0 |
...
6. **Voice has its first contextual reference, but is not yet conversational.**
   ...
   artwork and preserves brush meaning otherwise; mixed spoken corrections no
   longer disappear behind a leading yes/no. Broader references, multi-turn
