...
| Victor, 85    | Express an idea with dignity and confidence despite possible vision, tremor or memory limits; never fear losing work or wonder what mode/tool is active.                                          | Comfort mode, contrast, 44 px targets, reduced motion, voice-accessible steady strokes, honest text-contrast guidance, autosave and exact Undo.                                                                       | Comfort is hidden behind Settings; the adult shell is dense; no guided first journey, persistent task orientation, adaptive tremor profile or manual assistive-technology proof.                              |   6.0 |
| Ali, 30       | Preserve the feel of real media while gaining digital control, speed and production trust: expressive input, deep brushes, non-destructive experimentation, calibrated color and reliable output. | Pressure, calligraphy, visible stabilization, presets, layers with blend modes, editable adjustments and masks, selection, AI edits, PNG/JPEG.                                                                        | No tilt/azimuth, custom brush dynamics, stacked adjustment layers, color profiles, soft proofing, large-canvas performance proof or professional interchange.                                                 |   6.0 |
...
| Sara, 15      | Earn trust and income: create a distinctive logo quickly, revise it from client feedback, keep typography/colors precise, and deliver every professional file without embarrassment.              | Selection/layout, spacing grid, components, layers, named colors with text-contrast guidance, linked color variables, per-layer drop shadow, editable Bezier paths, honest SVG and one-click multi-size brand pack.   | No vector-node editing, typography depth, transparent-background workflow, rulers/custom guides, variants, PDF/print/CMYK delivery or client-review loop.                                                     |   5.5 |
| Zǐxuān, 28    | Communicate science accurately and publishably: preserve data integrity, build editable accessible figures/animations, use domain notation, and meet journal/export requirements.                 | Native grouped plots, scientific text/connectors, SVG, animation, text-contrast guidance, app links and a Chinese journey.                                                                                            | No units/error bars/log scales/fits, chemical structures, color-blind palette checks, scale bars, provenance, image-integrity record, editable PDF/EPS, DPI/physical sizing or publication preflight.         |   4.0 |
| Maria, 32     | Let agents reliably read, create, revise, render and ship real design assets inside coding workflows with typed contracts, transparent side effects and easy installation.                        | Portable `.dream`, stable engine, local MCP tools for read/render/export, named colors, linked color variables, layers, masks, strokes, shapes/text.                                                                  | Package not publicly installable; no raster/AI/component authoring, resources/prompts, output schemas, side-effect annotations, progress/cancellation, remote API/SDK or complete round-trip eval.            |   3.5 |
| Aleksandr, 25 | Move at professional thought speed: direct manipulation, precise inspectors, reusable systems, responsive layout, powerful shortcuts, extensibility and polished handoff without tool friction.   | Predictive pointer/drag states, keyboard workflows, exact grid/snapping/alignment, groups/components, layers, named colors, linked color variables, per-layer effect stack, editable Bezier paths and batch delivery. | No vector node editing, auto layout/constraints, design tokens, linked components/variants, rulers/custom guides, multi-page organization, extensibility or inspector depth.                                  |   4.0 |
...
The average is **4.9/10**. That is not a dismissal of the shipped breadth; it
...
30. **Voice should show its work and survive capability gaps.** Listening now
    has a clear state, growing transcript and interpreted phrase beside the
    result. Stop, retry and Escape are direct; permission and recognition
    failures stay localized and actionable through the identical typed intent
    path. The mic no longer becomes a dead control when a browser lacks speech.
31. **Clarification should expose the missing decision, not invent it.** “Move
    it” preserves the selected object and asks one bounded question without
...
37. **Assistance should reduce effort without taking the gesture away.** Steady
    ...
    control instead of a hidden correction. The same help is reachable by
    speech or typing without precise slider use, and its result remains visibly
    inspectable. Drawing stays tactually silent.
38. **A reusable choice and a linked system are different promises.** Project
    colors now give Sara, Ali, Fatima, Zǐxuān, Aleksandr and Maria one exact,
    named palette that survives handoff and is equally editable by people and
    agents. Recent colors remain personal and unnamed. Replacing a saved swatch
    deliberately leaves existing artwork alone, so this closes palette reuse
    and color precision—not variables, theme propagation or brand governance.
39. **A narrow measurement is more trustworthy than a broad badge.** Each
    project color now answers one concrete question with an exact number:
    normal-size opaque text against the current opaque canvas background. The
    adjacent AA/Below AA words make the state independent of status color, and
    the product deliberately does not imply that the same result validates
    artwork, transparency, large text, color vision or print. Agents receive
    the same derived facts instead of having to reverse-engineer the UI.
40. **Magnetism must never be invisible.** The Design grid starts hidden,
    exposes its exact interval and keeps its magnet separate from ordinary
    object/canvas snapping. When active, quiet lines become a full guide and an
    interval label at the pointer; the existing sparse detent reinforces that
    same visible event. Hiding the grid makes it non-magnetic, and the entire
    aid stays out of artwork and delivery. This closes basic spacing rhythm,
    not rulers, custom guides, layout constraints or responsive systems.
41. **A color variable is a link, not a bake.** A saved project color may be
    linked to a Design vector selection so the artwork follows the swatch's
    current value on the canvas, in SVG export and through MCP `render`. The
    link is one undoable step, recoloring clears it, and unlinking, deleting the
    swatch or importing a stale ref all freeze the op's last resolved color back
    into itself — severing a link never shifts the artwork. This is real token
    behavior for color, not spacing, typography or component variants.
...
6. **Voice has one real clarification, but is not broadly conversational.** The
   listening state, growing transcript, result, retry and typed fallback now
   expose the full one-turn loop instead of hiding it behind a pulsing mic.
   Natural
   ...
   spoken corrections no longer disappear behind a leading yes/no. “Move it”
   now asks a non-destructive direction question with visible choices, and the
   resolved nudge can safely continue through “again.” Typed and labelled
   recovery still assumes literacy; relationships between objects, broader
...
10. **Professional outcomes need the rest of the professional substrate, not
    ...
    visible stroke stabilization now combine with painted layer masks. The
    explicit Artwork/Mask and Hide/Reveal choices, crosshair and exact preview
    keep the destructive-risk target legible; stacked effects, vector paths,
    typography/color systems, publication preflight and richer agent schemas
    still unlock several personas at once.

## Recommended order

1. **P0 — truth and control:** tooltip, direct selection and visible/recoverable
   voice are deployed; keep real-browser gates for every reported failure.
...
4. **P1 — professional substrate:** build on shipped blend modes, editable
   per-layer adjustments, stroke stabilization, masks and portable named colors;
   continue with effect stacks and linked color variables, then vector paths,
   typography, grids/constraints and linked reusable systems—
